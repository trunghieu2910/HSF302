...
public class Main {
    private  static EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.mycompany.supperapp-PU");     // THÔNG SỐ CẤU HÌNH NHÀ THẦU JPA CHO JPA CLASS
   ...
   // EntityManager  là ông xếp quản lí các Entity --> Có thể CRUD  ==> Quản lý các Object tạo từ class Entity, sếp có thể thêm persit() , xóa remove() , cập nhật merge() , tìm kiếm theo key  : find() ==> vì ta luôn có nhu cầu thao tác trên 1 dòng cụ thể trong table    , truy vấn JPQL createQuery() --> để tìm linh hoạt theo tổ hợp điều kiện
    ...
    // KHI LÀM CÁC HÀNH ĐỘNG XOAAS / SỬA THÊM
    // TÂ PHẢI NHÉT NÓ VÀO TRANSACTION ĐỂ THEO DÕI : HƠACJ TẤT CẢ HƠACJ KHÔNG GÌ CẢ
    // NGUYÊN LÝ : DO OR NOTHING
    public static void update(){
        // KỸ THUTAAJ UPDATE ĐƠN GIẢN NHẤT / CÒN 1 KỸ THUẬT NỮA LÀ MERGE TÍNH SAU
    ...
    public static void selectAllLecturers() {
        ...
        List<Lecturer> lecturers = em.createQuery("SELECT lec FROM Lecturer lec WHERE lec.id=2 ", Lecturer.class).getResultList();
        // --> WHERE hardcode
        ...
        //QUERY ĐÔNGJ THAM SỐ WHAERE TRUYỀN TỪ WEB FORM ĐẾN ĐẤY CÓ 1 VALUE NÀO ĐÓ
        // JPQL  : SELECT x FROM Lecturer x WHERE x.salary = :pSalary
        // có quyền dùng OR , LIKE AND như sqql chuẩn
